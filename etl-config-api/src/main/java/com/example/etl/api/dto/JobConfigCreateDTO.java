...
// Add other validation imports as needed, e.g., for enums if we define them

public class JobConfigCreateDTO {

    @NotBlank(message = "Job name cannot be blank.")
    @Size(max = 255, message = "Job name cannot exceed 255 characters.")
    @Pattern(regexp = "^[a-zA-Z0-9_.-]+$", message = "Job name can only contain alphanumeric characters, underscores, dots, and hyphens.")
    ...
    private String sourceConnectionDetails; // Mandatory based on sourceType, checked in service/validator

    @Size(max = 50, message = "Source format cannot exceed 50 characters.")
    private String sourceFormat; // Mandatory based on sourceType
    ...
    private String sqlLogic; // CLOB as String, validated if mode is WITH_LOGIC/SCD2
    private String schemaMappingLogic; // CLOB as String (JSON)

    // SCD2 fields (optional, validated if mode is SCD2)
    ...
    private String reconciliationConfig; // CLOB as String (JSON)
    ...
    private String dqRulesConfig; // CLOB as String (JSON)
    ...
    private String jobProperties; // CLOB as String (JSON)
